...
def main():
    ...
    #object_pose = [0, 0.75]
    loaded_object_pose = [[0.08, -0.3], [0.08, -0.3], [0.08, -0.3]]
    unloading_range = [-6, 6]
    ...
    task_num = 1  #for each scene, the number of tested task
    ...
    for trial_idx in range(0, trial_num):
        ...
        arg = '$(find tamp_perception)/src/'+'ill.yaml'
        ...
        for task_idx in range(0, task_num):
            ...
            unloading_points = [(3, 6), (0.6, 6), (-3.5, 6)]
            for pick_idx in range(0, pick_n):
                ...
                unloading_point = unloading_points[pick_idx]
            ...
            print (optimal_plan)
            optimal_plan = ['navigate_to(linit,src,1)', 'load(o1,2)', 'load(o0,3)', 'load(o2,4)', 'navigate_to(src,l2_bot,5)', 'unload(o2,t2,6)', 'navigate_to(l2_top,l1_top,7)', 'unload(o1,t1,8)', 'navigate_to(l1_top,l0_top,9)', 'unload(o0,t0,10)']

            
            time.sleep(10)

            access_loc['l2_bot'] = Point(-3.7, 5.4375, 0)
            access_loc['l1_top'] = Point(0.625, 6.6875, 0)
            access_loc['l0_top'] = Point(2.625, 6.5625, 0)
            ...
            for a_idx in range(4, len(optimal_plan)): #already loaded object, start from the navigation action
                ...
                if a.split('(')[0] == 'unload':
                    ...
                    dx = loaded_object_pose[unload_num][0]
                    dy = loaded_object_pose[unload_num][1]
                    ...
                    if nav_goal != None: 
                        if side == "top":   ###### test2 for an unloading policy change for top side
                            ...
                            top_unloading[1] = unloading_point[1]
                ...
                #navigation action
                    ...
                    go_to_unloading(nav, side, nav_goal)
                    time.sleep(1)
            ...
            task_result['exec_cost'] = end_time-start_time
