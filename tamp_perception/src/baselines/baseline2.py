...
def unload_object(ac, object_pose, unloading_point):
    ...
    robot_pose = Point(-4, 5.5, 0)

    robot_orien = euler_to_quat(0, 0, 1.57)
...
def main():
    ...
    unload_object(ac, temp_unloaded, (-4, 6))
    return 
    ...
    random_plan = False
    sample_n = 1
    trial_num = 10  #scene change after each trial
    task_num = 5  #for each scene, the number of tested tasks
        ...
        for task_idx in range(0, task_num):
            ...
            for pick_idx in range(0, pick_n):
                ...
                for result in predicted_results:
                    ...
                    if side == "bot":
                        if random_sample == True or free_sample ==True:
            ...
            if random_plan == True:
                random_idx = random.sample(plan_idxs, 1)[0]
                optimal_plan = lines[random_idx].split(' ')
