...
def maybe_download_and_extract(dir_path,
                               url_name,
                               is_tarfile=False,
                               is_zipfile=False):
    ...
    if not os.path.exists(filepath):
        ...
        filepath, _ = urllib.request.urlretrieve(url_name,
                                                 filepath,
...
def save_image(image, save_dir, name, mean=None):
    ...
    #cv2.imwrtie(os.path.join(save_dir, name + ".png"), image)
...
def bottleneck_unit(x,
    ...
    def conv_transpose(tensor, out_channel, shape, strides, name=None):
        ...
        kernel = weight_variable([shape, shape, out_channel, in_channel],
                                 name=name)
        shape[-1] = out_channel
    ...
    def conv(tensor, out_chans, shape, strides, name=None):
        ...
        kernel = weight_variable([shape, shape, in_channel, out_chans],
                                 name=name)
