...
def main():
    ...
    for task_name in ["task6", "task7", "task8"]:
        for trial_idx in range(0, trial_num):
            ...
            if task_name in ["task8", "task6", 'task7']:
                task_nid = task_name.split('k')[-1]
                with open("gpt3_log/task_" + task_nid + "_c.json") as fp:
                    object_positions_candidates = json.load(fp)

                with open("gpt3_log/task_" + task_nid + "_b.json") as fp:
                    objects = json.load(fp)

                with open("gpt3_log/task_" + task_nid + "_a.json") as fp:
            ...
            for sample_n in range(sample_times):
                ...
                for obj in range(object_num):
                    this_side_feas = [10, 20, 30, 40]
                    object_side_feas.append(this_side_feas)
                    selected_seq.append(obj)
                ...
                for obj_idx in selected_seq:
                    ...
                    random.shuffle(this_object_feas)
            ...
            if "llm" not in os.listdir("exp/"):
                os.mkdir("exp/llm/")
            with open("exp/llm/" + task_name + '_' + str(trial_idx) + '.json',
                      'w+') as f:
            ...
            capture_config(
                ...
                "exp/llm/" + task_name + '_' + str(trial_idx) + '.png')
            cs.delete_all()
            sim_cs.delete_all()
