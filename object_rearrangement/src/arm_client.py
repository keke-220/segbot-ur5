...
class arm_client(object):
    ...
    def is_plan_found(self, robot_pose, robot_orien, ee_pose):
        ...
        pose_goal.position.x = (
            (dy - ry) * tan(theta) -
            (rx - dx)) / (cos(theta) + sin(theta) * tan(theta))
        pose_goal.position.y = (dy - ry - (dx - rx) * tan(theta)) / (
    ...
    def move_to_ee_pose_vertical(self, robot_pose, robot_orien, ee_pose):
...
if __name__ == '__main__':
    ...
    #print (robot_orien)
    goal_pose = Point(-0.1, 0, 1.3)
    test.is_plan_found(robot_pose, robot_orien, goal_pose)
    test.move_to_ee_pose(robot_pose, robot_orien, goal_pose)
    time.sleep(5)
    test.go_to_init_joints()
    #test.close_gripper()
