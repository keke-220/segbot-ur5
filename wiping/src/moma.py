...
INIT_ORIEN = euler_to_quat(0, 0, 3.14)
...
def draw_robot_position(ori_file, absolute_robot_pose, yaw, is_aug):
    robot_pose = []
...
def cmaes_objective(img_dict, dirty_mask, num_approach):
...
def move_arm(ac, freq):
    start = Point(-1.4, -4, 1.2)
    end = Point(-3.4, -4, 1.2)
    ee_traj = []
    ee_traj.append(start)
    batch = (start.x - end.x) / float(freq + 1)
    cur_ee = start.x
    for i in range(freq):
        ee_traj.append(Point(cur_ee - batch, -4, 1.2))
        cur_ee -= batch
    ee_traj.append(end)
    ...
    while n < len(ee_traj):
        pose = ee_traj[n]
        robot_pose = rospy.wait_for_message('/odom',
                                            Odometry).pose.pose.position
        robot_orien = rospy.wait_for_message('/odom',
                                             Odometry).pose.pose.orientation
        success = ac.move_to_ee_pose(robot_pose, robot_orien, pose)
        success = False
        n += 1


def move_base(nav, pose):
    nav.move_to_goal(pose, INIT_ORIEN)
...
def main():
    ...
    arm_control = Thread(target=move_arm, args=(ac, 40))
    base_control = Thread(target=move_base, args=(nav, Point(-3, -4, 0)))
    ...
    for trial_idx in range(NUM_TRIAL):
        ...
        for cp in chair_positions:
            std_chair_positions.append([cp[0] + 20, cp[1]])
        std_cs.spawn_table_no_sample([std_table_pose])
        std_cs.spawn_chair_no_sample(std_chair_positions, chair_oriens)
        """
