...
class chair_sampler(object):
    ...
    """
    def __init__(self, num_chair, table_pos, table_length, table_width):
        ...
        self._x_range = [table_pos[0]-table_length/2, table_pos[0]+table_length/2]
        self._y_range = [[table_pos[1]-(table_width/2)-1-r, table_pos[1]-(table_width/2)-r], [table_pos[1]+(table_width/2)+r, table_pos[1]+(table_width/2)+1+r]]
...
def main():
    test = chair_sampler(20, [0,0], 16, 1.0)
    #test.delete_all()
    test.spawn_both_sides()
    #time.sleep(3)
    #test.sample_pose()
