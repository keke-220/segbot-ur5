...
class scene_sampler(object):
    ...
    def set_table(self, table_pose):
        self._positions_table.append(table_pose)
    ...
    def sample_pose_chair(self, table_pose, table_size):
        num_sampled = 0
        while num_sampled < self._num_chair:
            #sample a random number within the x range
            x = random.uniform(table_pose[0] - table_size[0] / 2 - 0.5,
                               table_pose[0] + table_size[0] / 2 + 0.5)
            #sample y value
            y = random.uniform(table_pose[1] - table_size[1] / 2 - 0.5,
                               table_pose[1] + table_size[1] / 2 + 0.5)
    ...
    def spawn_chair(self, table_pose, table_size):
        self.sample_pose_chair(table_pose, table_size)
        try:
            spawner = rospy.ServiceProxy("/gazebo/spawn_sdf_model", SpawnModel)
        ...
        try:
            ...
            for i in range(0, len(p)):
                spawner(
                    ...
                    initial_pose=Pose(position=Point(p[i][0], p[i][1], 1.5),
                                      orientation=self.euler_to_quat(0, 0, 0)),
                    reference_frame="world")
